...
for (let i = 0; i < donor.length; i++) {
    for (let x = patient.length - 1; x >= 0; x--) {
        ...
        if (lengthDifference <= 0.2 * patientKidneyLength &&
            ...
            thicknessDifference <= 0.2 * patientKidneyThickness && donorBloodType == patientBloodType) {
            ...
            matchedusers.push(matched(patient[i].id, patient));
           ...
           const getMatch = JSON.stringify(matchedusers);
            ...
            fs.writeFile('matched.txt', getMatch, (err) => {
                console.log("Saved");
            });

            const donors = JSON.stringify(donor);
         
            fs.writeFile('donor.txt',donors, (err) =>{
                console.log("saved");           
             });
             ...
             fs.writeFile('patient.txt',patients, (err) =>{
                 console.log("saved");           
              });
        }
