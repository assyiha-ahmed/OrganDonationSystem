...
.then(data => {
    ...
    for (let i = 0; i < approved.length - 1; i += 2) {
        let donorApproved = approved[i];
        let patientApproved = approved[i+1];

        // console.log(donorApproved,patientApproved);

        createApproved(donorApproved.name, donorApproved.age, donorApproved.bloodType, patientApproved.name, patientApproved.age, patientApproved.bloodType);
    }
})
...
function createApproved(donor_name, donor_age,donor_bt,patient_name, patient_age,patient_bt){
...
const approvedContainer = document.querySelector(".approvedMatches");
approvedContainer.appendChild(singleMatchDiv);
}